...
- Live prescription OCR is **enabled** as a reviewed, lexicon-constrained pipeline (template OCR on a high-contrast raster, medical NER, official RxNorm CUIs). Unreadable images and tokens that do not resolve return `incomplete` (HTTP 409). Nothing is guessed from pixels. Fuzzy matches are unique, length-gated, and only against the lexicon. NLM RxNav is implemented and unit-tested with a stub; `liveRxnormNetwork` stays false until egress to rxnav.nlm.nih.gov is explicitly allowed.
- D5 clinical synthesis translates colloquial phrases only through `COLLOQUIAL_LEXICON`. Each clinical extra token must come from a mapping whose source span is in the patient transcript. Unmapped wording stays verbatim. Unsourced clinical terms (e.g. haematuria) still throw.
