...
- Live prescription OCR is **disabled** (`liveOcr: false`). C1 tests run on a messy OCR-transcript corpus that stands in for photographed scripts. An image upload without OCR must not invent medications. Landing copy says "type a prescription" rather than "photograph" so the claim matches the shipped engine.
